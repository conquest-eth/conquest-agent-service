...
class WithdrawalsStore extends BaseStore<Withdrawals> {
  ...
  getExits(): {location: string; stake: BigNumber}[] {
    return privateAccount.getSuccessfulExits().map((v) => {
      ...
      return {
        location: v,
        stake: BigNumber.from(planetInfo?.stats.stake).mul(
          '1000000000000000000'
        ),
      };
    });
  ...
  async check() {
    if (wallet.provider && wallet.address && wallet.contracts) {
      ...
      let balanceToWithdraw = await wallet.contracts.OuterSpace.callStatic.balanceToWithdraw(
        wallet.address
      );
      const exits = this.getExits();
      const locations = exits.map((v) => v.location);
      const planetsData = await wallet.contracts.OuterSpace.callStatic.getPlanetStates(
        locations
      );
      ...
      for (let i = 0; i < exits.length; i++) {
        ...
        const exit = exits[i];
        if (
          planet.owner.toLowerCase() !== wallet.address.toLowerCase() ||
          planet.exitTime === 0
        ) {
  ...
  async withdraw() {
    if (wallet.address && wallet.contracts) {
      ...
      const tx = await wallet.contracts.OuterSpace.fetchAndWithdrawFor(
        wallet.address,
        locations
      );
