...
class AgentStore extends BaseStore<Agent> {
  ...
  getNextFleet(): {fleetId: string; time: number} | undefined {
    ...
    return fleets.length > 0
      ? {time: fleets[0].arrivalTime, fleetId: fleets[0].id}
      : undefined;
  }
  ...
  async check() {
    if (wallet.provider && wallet.address && wallet.contracts) {
      if (
        ...
        wallet.address !== this.$store.ownerAddress
      ) {
        stop();
        this.setPartial({
          state: 'Idle',
  ...
  async resolveFleetsIfAny() {
    ...
    if (this.$store.nextFleet && now() > this.$store.nextFleet.time) {
      ...
      try {
        const contract = wallet.contracts.OuterSpace.connect(
          this.$store.wallet
        );
        this.lastTxTime = now();
        ...
        privateAccount.recordFleetResolvingTxhash(
          fleetId,
          tx.hash,
          tx.nonce,
          true
