<script lang="ts">
  ...
  function getVisited() {
    return lstorage.getItem('install-prompt') === 'true';
  }
  function setVisited() {
    lstorage.setItem('install-prompt', 'true');
  }
...
{#if deferredPrompt && show}
  <div
    ...
    <div
      class="max-w-sm w-full bg-gray-800 shadow-lg rounded-lg pointer-events-auto ring-1 ring-gray-100 ring-opacity-5">
      <div class="p-4">
        <div class="flex items-start">
          ...
          <div class="ml-3 w-0 flex-1">
            <p class="text-sm font-medium text-gray-100">Do you want to install conquest.eth on your home screen?</p>
          ...
          <div class="ml-4 flex-shrink-0 flex">
            <button
              on:click={decline}
              class="bg-gray-900 rounded-md inline-flex text-red-400 hover:text-red-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500">
