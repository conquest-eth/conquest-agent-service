...
contract ForwarderRegistry {
    ...
    bytes32 constant APPROVAL_TYPEHASH = keccak256(
        "ApproveForwarder(address signer,uint256 nonce,address forwarder,bool approved)"
    );
    ...
    function _encodeMessage(
        ...
        return
            abi.encodePacked(
                "\x19\x01",
                abi.encode(EIP712DOMAIN_TYPEHASH, EIP712DOMAIN_NAME, chainId),
                keccak256(abi.encode(APPROVAL_TYPEHASH, signer, nonce, forwarder, approved))
            );
    }
