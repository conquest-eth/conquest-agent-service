...
contract DAIPaymaster is BasePaymaster {
    ...
    function _callPermit(bytes memory context) internal {
        (address payer, uint256 nonce, uint256 expiry, uint8 v, bytes32 r, bytes32 s) = abi.decode(
            context,
    ...
    function _extractPermitArgs(bytes memory data)
        internal
        pure
        returns (
            uint256 nonce,
        ...
        )
    {
        ...
        assembly {
            s := mload(sub(add(data, length), 0x00))
        }
        assembly {
            r := mload(sub(add(data, length), 0x20))
        }
        assembly {
            v := mload(sub(add(data, length), 0x40))
        }
        // assembly { allowed := mload(sub(add(data, length), 0x60)) } // is true
        assembly {
            expiry := mload(sub(add(data, length), 0x60))
        }
        assembly {
            nonce := mload(sub(add(data, length), 0x80))
        }
