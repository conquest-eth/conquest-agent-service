...
export const TransactionInvalidFunctionSignature = () => errorResponse({code: 4000, message: "tx invalid, invalid function sig"});
export const TransactionInvalidNoData =() =>  errorResponse({code: 4001, message: "tx invalid, no data"});
export const TransactionInvalidUnrecoverableSIgner = () => errorResponse({code: 4002, message: "tx invalid, cannot recover signer"});
export const TransactionInvalidMissingFields = () => errorResponse({code: 4003, message: "duration, startTime, sendTxHash are all required"});
export const NonceAlreadyUsed = () => errorResponse({code: 4004, message: "nonce already used"});
export const TransactionCanAlreadyBeRevealed = () => errorResponse({code: 4101, message: "transaction can already be revealed"});
...
export const NotEnoughBalance = () => errorResponse({code: 4201, message: "Account have less than the minimum balance"}); // TODO parametrise to print balance required
export const InvalidNonce = () => errorResponse({code: 4300, message: "invalid nonce provided/signed"});

export const NoDelegateRegistered = () => errorResponse({code: 4400, message: 'No Delegate registered for that account'});
export const InvalidDelegate = () => errorResponse({code: 4401, message: 'Delegate not matching with the one registered with that account'});
