...
async function main() {
    ...
    const messageString = `conquest-agent-service: register ${registrationSubmission.delegate.toLowerCase()} as delegate for ${registrationSubmission.player.toLowerCase()} (nonce: ${registrationSubmission.nonceMsTimestamp})`;
    ...
    await request('queueReveal', {...revealSubmission, signature: queueSignature, delegate: delegateWallet.address.toLowerCase()});
}
